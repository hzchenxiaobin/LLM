...
对 logits 向量 $\mathbf{x} \in \mathbb{R}^C$，标准 softmax 为

$$\text{softmax}(\mathbf{x})_i = \frac{e^{x_i}}{\sum_{j=1}^{C} e^{x_j}}.$$

任取常数 $m$，都有恒等变形

$$\text{softmax}(\mathbf{x})_i = \frac{e^{x_i - m}}{\sum_{j=1}^{C} e^{x_j - m}}.$$

取 $m = \max_j x_j$，则所有指数 $x_j - m \le 0$，既避免溢出又便于分块时统一「基准」。

记当前行的**指数和**（分母）为

$$L = \sum_{j=1}^{C} e^{x_j - m}.$$

**2. 两段合并：新的全局最大值**

把下标分成先后两块（对应先看到一块 K、再看到下一块 K）。设已处理集合上的最大值为 $m^{\text{old}}$，指数为

$$L^{\text{old}} = \sum_{j \in \text{旧块}} e^{x_j - m^{\text{old}}}.$$

新来一块 logits，在该块内行内最大值为 $m^{\text{block}}$。定义**新的全局最大值**

$$m^{\text{new}} = \max\left(m^{\text{old}},\; m^{\text{block}}\right).$$

旧块里任意一项在新基准下应写成 $e^{x_j - m^{\text{new}}} = e^{x_j - m^{\text{old}}} \cdot e^{m^{\text{old}} - m^{\text{new}}}$，因此旧块对指数和的贡献整体乘上 $e^{m^{\text{old}} - m^{\text{new}}}$。新块直接按 $m^{\text{new}}$ 写指数。合并后

$$L^{\text{new}} = L^{\text{old}} \cdot e^{m^{\text{old}} - m^{\text{new}}} + \sum_{j \in \text{新块}} e^{x_j - m^{\text{new}}}.$$

这正是伪代码里「先缩放旧的 $l$，再加上新块的 $\sum \exp$」的来源。多块时从左到右反复套用同一式，即得**在线**更新，无需事先看到全部 $C$ 个 logits。

**3. 与 $V$ 的加权求和 $\mathbf{o} = \sum_i \text{softmax}_i \,\mathbf{v}_i$**

定义未归一化的分子（向量）

$$\tilde{\mathbf{o}} = \sum_{j} e^{x_j - m}\,\mathbf{v}_j,$$

则 $\mathbf{o} = \tilde{\mathbf{o}} / L$。当 $m$ 从 $m^{\text{old}}$ 更新到 $m^{\text{new}}$ 时，旧贡献同样整体乘以 $e^{m^{\text{old}} - m^{\text{new}}}$，新块直接加上 $\sum_{j \in \text{新块}} e^{x_j - m^{\text{new}}} \mathbf{v}_j$（矩阵形式即 $\mathbf{P}_{ij}\mathbf{V}_j$）。故

$$\tilde{\mathbf{o}}^{\text{new}} = e^{m^{\text{old}} - m^{\text{new}}}\,\tilde{\mathbf{o}}^{\text{old}} + \sum_{j \in \text{新块}} e^{x_j - m^{\text{new}}} \,\mathbf{v}_j.$$

若每一步都对 $\tilde{\mathbf{o}}/L$ 做归一化，就得到前文 V1 伪代码里「每一步除以 $l_{\text{new}}$」的写法；若只在整行所有块处理完后做一次 $\mathbf{o} = \tilde{\mathbf{o}}/L$，则对应 V2 把除法**推迟到行末**的写法——两者数学相同，只是缩放时机不同。

**4. 小结**

Online Softmax 的本质是：只维护 $(m,\, L,\, \tilde{\mathbf{o}})$ 三个量，每来一块 logits 就用新的 $\max$ 重定基准，对旧量做一次**指数 rescaling**，再叠加上新块的贡献；最后再 $\mathbf{o}=\tilde{\mathbf{o}}/L$。这样全程无需在内存里存放完整的 $N\times N$ 的 $S$ 或 $P$。
...
仔细看上面 V1 的伪代码中 $O_i$ 的更新公式：$\displaystyle\frac{O_i \odot l_i \odot \exp(m_i - m_{\mathrm{new}}) + P_{ij} V_j}{l_{\mathrm{new}}}$。
...
说明：`maximum`、`exp`、`sum` 等为按行向量运算；真实实现还会融合掩码、变长序列、$scale = 1/\sqrt{d}$ 等，且 $l_i$ 会加极小量避免除零。序列维并行体现在：不同的 $i$ 区间（不同 Q 块）可派发到不同 SM/线程块同时执行。
