...
步骤 1: 64 → 32（关键代码详解）
┌─────────────────────────────────────────────────────────────┐
│  代码: if (blockDim.x >= 64) sum = sdata[tid] + sdata[tid+32];│
│                                                              │
│  **为什么需要这个判断？**                                      │
│  • 前面的共享内存归约循环条件: s > 32                          │
│    → 当 s = 32 时就停止，此时还剩下 32 或 64 个元素             │
│  • 如果 blockSize = 256: 循环后剩 64 个元素 (sdata[0..63])      │
│  • 如果 blockSize = 32: 循环不执行，直接到这里，剩 32 个元素    │
│                                                              │
│  **两种情况处理:**                                             │
│  • blockDim.x >= 64: 需要先把 64→32，sum = sdata[tid] +        │
│                      sdata[tid+32]                            │
│  • blockDim.x < 64:  直接取 sdata[tid]，无需额外归约           │
│                                                              │
│  示例 (blockSize=256):                                        │
